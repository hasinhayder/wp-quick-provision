...
add_action( 'admin_menu', function () {
	add_menu_page( 'WP Quick Provision', 'Quick Provision', 'manage_options', 'wpqp', function () {
        ...
        <form method="POST">
            ...
            <input type="text" style="width:60%;font-size: 15px;margin-top:10px; line-height: 30px;" name="gist"
                   id="gist" cols="70"
                   rows="5" placeholder="<?php _e( 'Gist URL with Provision Data', 'wp-quick-provision' ); ?>"/><br/>
			<?php
			if ( isset( $_POST['submit'] ) ) {
				...
				if ( strpos( $wpqp_gist_url, "gist" ) === false ) {
					$wpqp_proceed = false;
					?>
                    <div class="notice notice-error is-dismissible" style="margin-left: 1px;">
			...
			if ( count( $wpqp_themes ) > 0 ) {
				...
				foreach ( $wpqp_themes as $wpqp_theme ) {
					...
					if ( ! array_key_exists( $wpqp__theme, $wpqp_installed_themes ) ) {
						?>
                        <div class="notice notice-success" style="margin-left: 1px;">
						...
						?>
                        <div class="notice notice-error" style="margin-left: 1px;">
